...
function Element:New(Config)
    ...
    function Slider:Update(UpdateConfig)
        UpdateConfig = UpdateConfig or {}
        
        -- Update Title if provided
        if UpdateConfig.Title ~= nil then
            Slider.Title = UpdateConfig.Title
            if Slider.SliderFrame.UIElements.Title then
                Slider.SliderFrame.UIElements.Title.Text = UpdateConfig.Title
            end
        end
        
        -- Update Desc if provided
        if UpdateConfig.Desc ~= nil then
            Slider.Desc = UpdateConfig.Desc
            if Slider.SliderFrame.UIElements.Desc then
                Slider.SliderFrame.UIElements.Desc.Text = UpdateConfig.Desc
                -- Update visibility based on desc content
                Slider.SliderFrame.UIElements.Desc.Visible = (UpdateConfig.Desc ~= "" and UpdateConfig.Desc ~= nil)
            end
        end
        
        -- Update Value if provided (Min, Max, Default)
        if UpdateConfig.Value ~= nil then
            if UpdateConfig.Value.Min ~= nil then
                Slider:SetMin(UpdateConfig.Value.Min)
            end
            if UpdateConfig.Value.Max ~= nil then
                Slider:SetMax(UpdateConfig.Value.Max)
            end
            if UpdateConfig.Value.Default ~= nil then
                Slider:Set(UpdateConfig.Value.Default)
            end
        end
    end
